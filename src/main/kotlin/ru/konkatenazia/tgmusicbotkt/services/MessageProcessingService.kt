...
class MessageProcessingService(
    ...
    fun isEnglishWord(wordForCheck: String): Boolean {
        ...
        for (word in splittedWords) {
            try {
                val response = restTemplate.getForEntity(url + word, WordDTO::class.java)
                val insultWords = response.body

                if (insultWords != null && response.statusCode.is2xxSuccessful) {
                    val count = arrayOf(insultWords)
                        .filter { elem -> elem.meanings.isNotEmpty() }
                        .filter { elem -> elem.meanings[0].definitions.isNotEmpty() }
                        .map { elem -> elem.meanings[0].definitions[0].definition }
                        .distinct()
                        .count()
